...
| DeepSeek V4 | 作为计划中的核心LLM引擎，默认使用 `deepseek-v4-pro`，用于后续接入摘要、评分、分类和内容生成 |
...
当前版本为了保证用户测试稳定，DeepSeek V4接口采用统一客户端与离线fallback设计，核心逻辑以离线可复现方式实现。后续可以在不重做数据库结构的情况下替换为真实LLM调用。
...
2. 接入DeepSeek V4，替换当前离线摘要、评分、分类和内容生成逻辑。
