...
| 阶段六 | `0_main_workflow.py` | 全部阶段脚本 | 主控日志、最终报告支撑数据 | 一键串联阶段二到阶段五，并进行数据库健康检查 |
...
| 审计运行记录 | `ingestion_runs`、`routing_runs`、`classification_runs`、`kol_analysis_runs`、`linkedin_content_runs` | 保存每次运行的数量统计、错误数与运行ID |
